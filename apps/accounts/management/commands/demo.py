...
class AgentThread(Thread):
    ...
    def __init__(
        self, user_name="demo_user", seed=0, min_sleep=1, max_sleep=10, **kwargs
    ):
    ...
    def execute_action(self, action):
        if (len(CompetitionModel.objects.all()) == 0) and (
            ...
            response = self.launch_competition(
                ...
                problem_type=random.choice(["classification", "regression"]),
        ...
        if action == "access_page":
            ...
            response = self.client.post(
                reverse("Discussion:discussion_competition", args=[comp.id]),
                {
                    ...
                    "comment_field_disc": self.random_words(30, period=True),
                },
            )
        elif action == "comment":
    ...
    def random_words(self, num_words, period=False):
        return " ".join([self.r.get_random_word() for _ in range(num_words)]) + (
            "." if period else ""
        )
    ...
    def launch_competition(
        ...
        open_datetime = timezone.make_aware(
            timezone.datetime.now() + timedelta(minutes=np.random.randint(1))
        )
        close_datetime = open_datetime + timedelta(minutes=np.random.randint(2) + 1)
        # 提出例ファイル
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        staff_users = [
            AgentThread(
                user_name=f"__demo__staff_user{i}",
                is_superuser=False,
                is_staff=True,
                is_participant=True,
                min_sleep=1,
        ...
        super_users = [
            AgentThread(
                user_name=f"__demo__super_user{i}",
                is_superuser=True,
                is_staff=True,
                is_participant=True,
                min_sleep=1,
