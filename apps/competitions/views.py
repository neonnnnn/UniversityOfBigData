    ...
    def get_context_data(self, **kwargs):
        ...
        for st in status_list:
            ...
            for q_set in query_sets:
                ...
                # 投稿数、参加者数、参加チーム数
                post_set = CompetitionPost.objects.filter(
                    post=q_set,
                )
                competition_dict["post_count"] = len(post_set)
                team_set = (
                    post_set.order_by("team_tag").distinct().values_list("team_tag")
                )
                competition_dict["team_count"] = len(team_set)
                user_set = (
                    post_set.order_by("user_tag").distinct().values_list("user_tag")
                )
...
class CompetitionsRankingView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        # 管理用順位付け
        rank_list = rankdata(
            [is_reversed * i for i in score_list], method="min"
        ).astype(int)
        final_rank_list = rankdata(
            [is_reversed * i for i in fin_score_list], method="min"
        ).astype(int)

        # 管理用
        score_management_box = []
        for post_obj, score, f_score, rank_id, f_rank_id in zip(
            post_tags, score_list, fin_score_list, rank_list, final_rank_list
        ):
            ...
            b2 = [
                ...
                str(post_obj.count_par_today) + " / " + str(post_obj.count_posts),
            ]
        ...
        show_rank_list = rankdata(
            [is_reversed * i for i in post_score], method="min"
        ).astype(int)
        show_final_rank_list = rankdata(
            [is_reversed * i for i in post_fin_score], method="min"
        ).astype(int)
        # 可視化表用並び替え
        score_box = []
        score_fin_box = []
...
class CompetitionsPostView(LoginRequiredMixin, FormMixin, DetailView):
    ...
    def get_success_url(self):
        # logging
        return reverse_lazy(
            "Competitions:competitions_ranking", kwargs={"pk": self.object.pk}
        )
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["n_sub_daily"] = count_daily_submissions(self.request.user, Competition)
        context["post_tag"] = self.get_object().title
        context["team_tag"] = (
            self.request.user.selectedTeam.name
            if self.request.user.selectedTeam
            else "unknown"
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        n_day_sub = count_daily_submissions(request.user, self.get_object())
