...
def saveSubmissionPath(instance, filename):
    ...
    user_nickname = instance.user_tag.nickname if instance.user_tag else "unknown-user"
    new_name = team_name + "_" + user_nickname + "_Post"
    date_text = datetime.date.today()
    date_text = str(date_text)
    date_text = date_text.replace("-", "")
    post_count = (
        instance.count_par_today
    )  # 同一チームで本日何個投稿しているか？のカウント
...
class CompetitionModel(models.Model):
    title = models.CharField(
        _("コンペティションのタイトル（日本語）"), max_length=150, blank=False
    )
    ...
    use_forum = models.BooleanField(_("フォーラムの使用"), default=False)  # 不要？
    added_datetime = models.DateTimeField(
        _("コンペティション登録日"), auto_now_add=True
    )  # 非表示
    ...
    n_max_submissions_per_day = models.IntegerField(_("１日の最大投稿回数"), default=3)
    ...
    truth_blob_key = models.FileField(
        _("正解データファイル"), upload_to=saveGroundTruthPath
    )
    ...
    winner_teams = models.ManyToManyField(
        to=TeamTag, related_name="winner_team", verbose_name=_("優勝チーム"), blank=True
    )
    ...
    displayed_decimal_places = models.IntegerField(
        _("表示されるスコアの小数点以下の桁数"), default=5, blank=True
    )  # 投稿数
    ...
    def evaluate_submission(self, submission_file_path: str) -> Tuple[Real, Real]:
        """Public LBおよびPrivate LBの評価指標を返す."""
        return metrics[self.evaluation_type](self.public_leaderboard_percentage / 100)(
            self.truth_blob_key, submission_file_path
        )

    def displayed_score(self, score: models.DecimalField) -> str:
        return ("{" + f":.{self.displayed_decimal_places}f" + "}").format(score)
