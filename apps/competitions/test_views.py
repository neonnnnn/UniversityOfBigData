        ...
        compes.append(
            CompetitionModel.objects.create(
                ...
                close_datetime=timezone.make_aware(
                    timezone.datetime(2023, 7, 31, 18, 0)
                ),
                status="active",
            )
        )
...
class CompetitionViewsTests(TestCase):
    ...
    def _test_view_submission(self, m_name, gt, preds, expected_scores):
        ...
        expected_ranks = rankdata(
            -(2 * metrics[m_name].greater_is_better - 1) * np.array(expected_scores),
            method="min",
        )
        ...
        for i in range(preds.shape[0]):
            user_name = (
                etree.HTML(res_get.content.decode())
                .cssselect("table")[0][0][i + 1][1]
                .text.split("/")[1]
                .strip()
            )
            actual_rank = int(
                etree.HTML(res_get.content.decode())
                .cssselect("table")[0][0][i + 1][0]
                .text
            )
    ...
    FLOAT_PREDS = np.array(
        [
            ...
            [-8.117, -6.561, 5.734, 4.266, 0.423, -7.415, -1.12, 9.493, -5.174, -5.373],
    ...
    @parametrize(
        ...
        [
            ...
            ("accuracy", INT_GT, INT_PREDS, [0.1, 0.2, 0.1]),
            ("recall", np.mod(INT_GT, 2), np.mod(INT_PREDS, 2), [0.4, 0.5, 0.5]),
            (
                "precision",
                np.mod(INT_GT, 2),
                np.mod(INT_PREDS, 2),
    ...
    def _test_wrong_formatted_submisson(
        self, m_name="accuracy", pred=np.arange(10), gt=np.arange(10), expected_score=1
    ):
