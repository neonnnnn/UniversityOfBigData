...
def setplan01():
    ...
    for comp in compALL:
        ...
        if now_time < open_date:
            ...
            # 開催
            word = (
                _("コンペティション") + " " + comp.title + " " + _("開催準備中です。")
            )
            ...
            """
            word = (
                _("コンペティション") + " " + comp.title + " " + _("が終了しました。")
            )
            ...
            # 開始ログ発出
            word = (
                _("コンペティション") + " " + comp.title + " " + _("が始まりました。")
            )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        for t in self.option_keys:
            parser.add_argument("--" + t, default="*", type=str, required=False)
    ...
    def handle(self, *args, **options):
        ...
        scheduler.add_job(
            setplan01,
            trigger=CronTrigger(**{k: options[k] for k in self.option_keys}),
            id="setplan01",
            max_instances=1,
