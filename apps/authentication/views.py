...
def prepare_top_context(request):
    ...
    # コンペティション数
    pub_competition_num = CompetitionModel.objects.all().filter(public=True).count()
    closed_competition_num = CompetitionModel.objects.all().filter(public=False).count()
    # 時刻
    ...
    for log_entry in reversed(log_data):
        if len(activity_logs) >= num_activity_logs:
            break
        if ("user_name" in log_entry.keys()) and (user_name == log_entry["user_name"]):
    ...
    for log_entry in reversed(log_data):
        ...
        if (
            ...
            msg = (
                f"""{log_entry['user_name']} {_('の活動: ')} """
                f"""{log_entry['message']}"""
            )
            if (last_msg is None) or (last_msg != msg):
                other_user_logs.append(
                    {"time": parse_datetime(log_entry["time"]), "activity": msg}
                )
                last_msg = msg
...
def login_required(request):
    ...
    return render(request, "login_required.html", {"form": form})
