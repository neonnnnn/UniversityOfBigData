...
class Discussion(models.Model):
    title_disc = models.CharField(
        max_length=128, blank=False, null=False, verbose_name=_("題名")
    )
    post_tag_disc = models.ForeignKey(
        to=CompetitionModel,
    ...
    team_tag_disc = models.ForeignKey(
        to=TeamTag,
        on_delete=models.SET_NULL,
        related_name="discussion_team_tag",
        verbose_name=_("投稿チーム"),
        null=True,
    )
    user_tag_disc = models.ForeignKey(
        to=User,
        on_delete=models.SET_NULL,
        related_name="discussion_user_tag",
        verbose_name=_("投稿者"),
        null=True,
    )
    comment_field_disc = models.TextField(
        blank=False, null=False, verbose_name=_("コメント")
    )
    # 情報控え(ユーザーやチームが消えた時の処理用)
    author_user_nickname = models.CharField(
        max_length=64,
        blank=False,
        null=False,
        verbose_name=_("投稿者名"),
    ...
    author_user_id = models.CharField(
        max_length=64, blank=False, null=False, verbose_name=_("投稿者ID"), default="0"
    )
    author_team_name = models.CharField(
        max_length=64,
        blank=False,
        null=False,
        verbose_name=_("投稿チーム"),
        default="anonymous term",
    )
    author_team_id = models.CharField(
        max_length=64,
        blank=False,
        null=False,
        verbose_name=_("投稿チームID"),
        default="0",
    )
...
class DiscussionPost(models.Model):
    post_tag_post = models.ForeignKey(
        to=Discussion,
        on_delete=models.CASCADE,
        related_name="discussion_post_tag",
        verbose_name=_("議論板"),
    )
    added_datetime_post = models.DateTimeField(_("投稿日"), auto_now_add=True)
    team_tag_post = models.ForeignKey(
        to=TeamTag,
        on_delete=models.SET(get_deleted_team),
        related_name="post_team_tag",
        verbose_name=_("投稿チーム"),
        null=True,
    )
    user_tag_post = models.ForeignKey(
        to=User,
        on_delete=models.SET(get_deleted_user),
        related_name="post_user_tag",
        verbose_name=_("投稿者"),
        null=True,
    )
    comment_field_post = models.TextField(
        blank=False, null=False, verbose_name=_("コメント")
    )
    # 情報控え(ユーザーやチームが消えた時の処理用)
    author_user_nickname = models.CharField(
